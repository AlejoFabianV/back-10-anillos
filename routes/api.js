...
router.get('/novedades', async function (req, res, next) {
    ...
    novedades = novedades.map(novedades => {
        if (novedades.img_id) {
          const portada = cloudinary.url(`https://res.cloudinary.com/dbouwy1jn/image/upload/${novedades.img_id}.jpg`, {
            width: 90,
            height:120,
            crop: 'fill',
            secure: true
...
router.get('/producto/:titulo/:subtitulo/:id', async function (req, res, next) {
  ...
  let portada = cloudinary.url(`https://res.cloudinary.com/dbouwy1jn/image/upload/${producto.img_id}.jpg`, {
    secure: true
  });
  producto = {
        ...producto,
